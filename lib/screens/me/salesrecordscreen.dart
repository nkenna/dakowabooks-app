...
class _SalesRecordScreenState extends State<SalesRecordScreen> {
  ...
  void initState() {
    ...
    scrollController.addListener(() async{
      if(scrollController.position.pixels >= scrollController.position.maxScrollExtent){
        ...
        print("total all literature page: ${ Provider.of<LiteratureProvider>(context, listen: false).salesRecordTotalPages}");
        print("current page: ${ currentPage}");
        if(currentPage <= Provider.of<LiteratureProvider>(context, listen: false).salesRecordTotalPages){
          final dd = await Provider.of<LiteratureProvider>(context, listen: false).salesRecordByAuthor(
              currentPage,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text("${wt.userBuyer!.email}", style: TextStyle(color: mainTextColor, fontSize: 14, fontFamily: 'SofiaProRegular'),),
  ...
  Widget build(BuildContext context) {
    return SafeArea(
        ...
        child: Scaffold(
          appBar: AppBar(
            title: Text("Your Sales Record", style: TextStyle(fontFamily: 'MonumentRegular', fontSize: 14),),
          ),
